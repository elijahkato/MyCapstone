...
schema_view = get_schema_view(
   openapi.Info(
      ...
      contact=openapi.Contact(email="elijahosas@gmail.com"),
...
urlpatterns = [
    ...
    path('api/', include('inventory_app.urls')),  # Inventory
    
    # JWT Token URLs
    path('api/token/', TokenObtainPairView.as_view(), name='token_obtain_pair'),  # Obtain a new token
    path('api/token/refresh/', TokenRefreshView.as_view(), name='token_refresh'),  # Refresh the token
    path('api/token/verify/', TokenVerifyView.as_view(), name='token_verify'),  # Verify the token
